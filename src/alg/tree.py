...
class TreeNode:
    def __init__(self, ka, kv, res, lc, rc):
    ...
    def build(self, xs, ys):
        ...
        if best_gain > 0:
            return TreeNode(ka=best_attr, kv=best_val, res=None,
                            lc=self.build(*best_split[0]), rc=self.build(*best_split[1]))
        else:
            return TreeNode(ka=None, kv=None, lc=None, rc=None, res=Counter(ys))
